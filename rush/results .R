...
# Push running tasks
reg = loadRegistry(file.dir = file.path(registry, "push_running_tasks"), writeable = TRUE)
...
fwrite(job_table, "rush/results_push_running_tasks.csv")

# Finish tasks
reg = loadRegistry(file.dir = file.path(registry, "finish_tasks"), writeable = TRUE)
...
reg = loadRegistry(file.dir = file.path(registry, "fetch_finished_tasks"), writeable = TRUE)
...
fwrite(job_table, "rush/results_fetch_finished_tasks.csv")  

# Fetch finished tasks with cache
reg = loadRegistry(file.dir = file.path(registry, "fetch_finished_tasks_cache"), writeable = TRUE)
...
job_table[, list(n_tasks, n_parameters, payload_size, runtime_median, runtime_mad)]
fwrite(job_table, "rush/results_fetch_finished_tasks_cache.csv")
