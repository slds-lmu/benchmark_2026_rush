...
initial_designs = readRDS("initial_designs.rds")
...
central_mbo = function(task, learner, resampling, measure, terminator, initial_design, n_workers, config, ...) {
  ...
  tuner = tnr("async_mbo_central", initial_design = initial_design)
...
# algorithm = async_mbo
# algorithm_name = "async_mbo"
# otask_id = 3L
# initial_design = initial_designs[[1]]
# profile = NULL

results = pmap(list(otask_id = c(31L, 3945L, 7592L, 189354L), initial_design = initial_designs), function(otask_id, initial_design) {
  ...
  mlr3misc::imap(list("cl_mbo" = cl_mbo, "central_mbo" = central_mbo, "async_mbo" = async_mbo), function(algorithm, name) {
    ...
    while (mirai::status(.compute = profile)$connections < n_workers) {
      Sys.sleep(10)
      mlr3misc::messagef("Waiting for workers to connect... %i/%i", mirai::status(.compute = profile)$connections, n_workers)
    }

    archive = mlr3misc::invoke(algorithm, task = task, learner = learner, resampling = resampling, measure = measure, terminator = terminator, initial_design = initial_design, n_workers = n_workers, config = config)
