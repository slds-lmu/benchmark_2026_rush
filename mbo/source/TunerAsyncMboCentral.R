#' @title TunerAsync using Centralized Asynchronous Model Based Optimization
#'
#' @include OptimizerAsyncMboCentral.R
#' @name mlr_tuners_async_mbo_central
#'
#' @description
#' `TunerAsyncMboCentral` class that implements Centralized Asynchronous Model Based Optimization.
#' This is a minimal interface internally passing on to [OptimizerAsyncMboCentral].
#' For additional information and documentation see [OptimizerAsyncMboCentral].
#'
#' Currently, only single-objective optimization is supported and `TunerAsyncMboCentral` is considered an experimental feature and API might be subject to changes.
#'
#' @section Parameters:
#' \describe{
...
#'
#'     tnr("async_mbo_central", design_size = 4, n_workers = 2)$optimize(instance)
#'     mirai::daemons(0)
#'   } else {
...
#' }
#' }
TunerAsyncMboCentral = R6Class("TunerAsyncMboCentral",
  ...
  public = list(
    ...
    #'
    #' Note that all the parameters below are simply passed to the [OptimizerAsyncMboCentral] and
    #' the respective fields are simply (settable) active bindings to the fields of the [OptimizerAsyncMboCentral].
    #'
    ...
    initialize = function(surrogate = NULL, acq_function = NULL, acq_optimizer = NULL, param_set = NULL) {
      optimizer = OptimizerAsyncMboCentral$new(surrogate = surrogate, acq_function = acq_function, acq_optimizer = acq_optimizer, param_set = param_set)

      super$initialize(optimizer = optimizer, man = "mlr3mbo::TunerAsyncMboCentral")
    },
